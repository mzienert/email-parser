...
/**
 * BedrockClient - AWS Bedrock client wrapper for LLM integration
 * 
...
class BedrockClient {
   ...
   */
  static DEFAULT_MODEL = BedrockClient.MODELS.CLAUDE_3_7_SONNET;
...
Provide only the JSON response with no additional text or explanation.`
  ...
  static async invokeModel(bedrockClient, prompt, modelId = null, options = {}) {
    const selectedModel = modelId || BedrockClient.DEFAULT_MODEL;
    ...
    const payload = {
      ...
      system: BedrockClient.PROMPT_TEMPLATES.SYSTEM_PROMPT,
      messages: [
        {
          role: "user",
          content: prompt
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Parse JSON response
        return BedrockClient.parseModelResponse(content);
  ...
  static parseModelResponse(content) {
    try {
      ...
      return {
        ...parsed,
        bedrockConfidence: BedrockClient.calculateConfidence(parsed),
  ...
  static constructExtractionPrompt(emailData, parserType, responseFormat) {
    const specializedInstructions = BedrockClient.getSpecializedInstructions(parserType);
    
    let prompt = BedrockClient.PROMPT_TEMPLATES.EXTRACTION_PROMPT
